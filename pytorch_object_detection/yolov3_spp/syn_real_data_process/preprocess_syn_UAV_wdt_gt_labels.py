...
def split_real_wdt_trn_val(seed=17, comment='syn_color', pxwhr='px15whr4', suffix='_xcycwh'):
    annos_dir = syn_args.real_annos_dir
    annos_files = glob.glob(os.path.join(annos_dir, '*.txt'))
    imgs_dir = syn_args.real_img_dir
...
def get_args(cmt='', real_folder='xilin_wdt'):
    ...
    parser.add_argument("--val_percent", type=float, default=0.3, help="train:val=0.8:0.2")
...
if __name__ == '__main__':
    ...
    # split_real_wdt_trn_val(comment=comment, pxwhr=pxwhr, suffix='_xcycwh')
    # ####### create *.data
