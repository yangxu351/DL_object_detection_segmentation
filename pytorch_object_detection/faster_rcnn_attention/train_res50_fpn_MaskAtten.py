...
import transforms
from network_files.faster_rcnn_framework_MaskAtten import FasterRCNN, FastRCNNPredictor
from backbone.resnet50_fpn_ART import resnet50_fpn_backbone
from my_dataset import VOCDataSet
from train_utils import GroupedBatchSampler, create_aspect_ratio_groups
from train_utils import train_eval_utils_MaskAtten as utils
...
def create_model(num_classes, parser_data):
    ...
    model = FasterRCNN(backbone=backbone, num_classes=91, withRPNMask=parser_data.withRPNMask, soft_val=parser_data.soft_val,  withMaskFeature=parser_data.withMaskFeature, layer_levels=parser_data.layer_levels) 
    # 载入预训练模型权重
...
def main(parser_data, dir_args, train_syn=True):
    ...
    if train_syn:
        ...
        if parser_data.withMaskFeature or parser_data.withFPNMask or parser_data.withRPNMask:
    ...
    for epoch in range(parser_data.start_epoch, parser_data.epochs):
        ...
        if epoch % 10 == 0 or epoch >= parser_data.epochs - 1:
            ...
            torch.save(save_files, os.path.join(parser_data.weight_dir, "resNetFpnMaskAtten-model-{}.pth".format(epoch)))
...
if __name__ == "__main__":
    ...
    from parameters_MaskAtten import *
    ...
    # 是否 withMaskFeature
    parser.add_argument('--withMaskFeature', default=WITH_MASK_FEATURE, type=bool, help='True when training withMaskFeature. ')
    ...
    if args.withFPNMask:
        ...
        folder_name = f'{time_marker}_lr{args.lr}_bs{args.batch_size}_{args.epochs}epochs_MaskFeat{args.withMaskFeature}_softval{args.soft_val}_{len(args.layer_levels)}layer_levels'
    else:
