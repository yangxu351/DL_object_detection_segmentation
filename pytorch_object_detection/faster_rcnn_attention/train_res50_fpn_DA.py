...
def create_model(num_classes, parser_data):
    ...
    backbone = resnet50_fpn_backbone(norm_layer=torch.nn.BatchNorm2d, returned_layers=[1,2,3,4],
                                     trainable_layers=3, lc=parser_data.lc, gl=parser_data.gl, context=parser_data.context)
    # 训练自己数据集时不要修改这里的91，修改的是传入的num_classes参数
    model = FasterRCNN(backbone=backbone, num_classes=91, withRPNMask=parser_data.withRPNMask, soft_val=parser_data.soft_val, lc=parser_data.lc, gl=parser_data.gl, context=parser_data.context) #, min_size=parser_data.input_size
...
def main(parser_data, syn_dir_args, real_dir_args):
    ...
    for epoch in range(parser_data.start_epoch, parser_data.epochs):
        ...
        print("[epoch %2d] loss: %.4f, lr: %.2e,  eta: %.4f"  % (epoch, mean_loss.item(), lr, args.eta))
