...
def main(parser_data, val_all=False):
    ...
    txt_name = "all.txt" if val_all else f"val.txt"
    ...
    model = FasterRCNN(backbone=backbone, num_classes=parser_data.num_classes + 1, withRPNMask=parser_data.withRPNMask, soft_val=parser_data.soft_val, withMaskFeature=parser_data.withMaskFeature)
    # 载入你自己训练好的模型权重
...
if __name__ == "__main__":
    ...
    # real_cmt = 'DJI_wdt'
    epc = 49
    folder_name = '20221010_0739_lr0.0001_bs4_50epochs_MaskFeatTrue_softval-1_2layers_bn_softmax_art_nopool_update' # 0.3383 (val) # 0.3032 (all)
