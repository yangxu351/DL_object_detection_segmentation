...
class RPNMaskwithFeaturesHead(nn.Module):
    """
    ...
    def __init__(self, in_channels, num_anchors, soft_val=1.0):
        super(RPNMaskwithFeaturesHead, self).__init__()
        # 3x3 滑动窗口
        self.conv = nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=1, padding=1)
        # 计算预测的目标分数（这里的目标只是指前景或者背景）
        self.cls_logits = nn.Conv2d(in_channels, num_anchors, kernel_size=1, stride=1)
    ...
    def forward(self, x, masks=None):
        # type: (Dict[str, Tensor], List[Tensor]) -> Tuple[List[Tensor], List[Tensor], Dict[str, Tensor]]
        logits = []
        bbox_reg = []
        features = {}
        for k, feature in x.items():
            # t = F.relu(self.conv(feature))
            if masks is not None and (k=='0' or k=='1'):
...
class RegionProposalNetwork(torch.nn.Module):
    ...
    def forward(self,
        ...
        # type: (...) -> Tuple[List[Tensor], Dict[str, Tensor], Dict[str, Tensor]]
        """
        Arguments:
        ...
        Returns:
            ...
            features (Dict[str, Tensor]): the attention region transfered features
        """
        # RPN uses all feature maps that are available
        # features是所有预测特征层组成的OrderedDict
        # tag: yang comment for ART
        ...
        if masks is not None: 
            ...
            # tag: yang changed, return the mask filtered features
            objectness, pred_bbox_deltas, art_features = self.head(features, masks) # RPNMaskwithFeaturesHead
            #fixme: whether use art_features to filter the features
        ...
        if masks is not None: 
            return boxes, losses, art_features
        else:
            return boxes, losses
