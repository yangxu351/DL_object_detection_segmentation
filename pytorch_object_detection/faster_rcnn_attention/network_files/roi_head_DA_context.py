...
class RoIHeads(torch.nn.Module):
    ...
    def forward(self,
                ...
                lc_features=None,    # type: Dict[str, Tensor]
                gc_features=None,    # type: Dict[str, Tensor]
                ):
        ...
        # feature shape: level_layers*[batch_size, chanel, height, width] # 4*[8, 256, 100, 100]
        # proposals: batch_size*[sampled_proposals, 4]  8*[512, 4], 8*512=4096
        # image shape: [800, 800]
        # box_features_shape: [num_proposals, 256, 7, 7] # [4096, 256, 7, 7]
        box_features = self.box_roi_pool(features, proposals, image_shapes)

        # 通过roi_pooling后的两层全连接层
        # box_features_shape: [num_proposals, representation_size]
        box_features = self.box_head(box_features) # [4096, 1024]
        
        # tag: yang adds lc context + gc context
        # features 8, 256, 200, 200
        if lc_features is not None:
            for lc_feat in lc_features.values():# 8, 64, 1, 1 proposals.size(1)=512
                bs = lc_feat.shape[0]
                num_p = box_features.shape[0]//bs
                lc_feat = lc_feat.view(bs, -1).repeat(num_p, 1)
                box_features = torch.cat((lc_feat, box_features), 1)
            for gc_feat in gc_features.values():
                bs = gc_feat.shape[0]
                num_p = box_features.shape[0]//bs
                gc_feat = gc_feat.view(bs, -1).repeat(num_p, 1)
                box_features = torch.cat((gc_feat, box_features), 1)    
