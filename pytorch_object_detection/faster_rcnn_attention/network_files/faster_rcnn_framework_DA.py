...
class FasterRCNNBase(nn.Module):
    ...
    def forward(self, images, targets=None, masks=None, is_target=False):
        # type: (List[Tensor], Optional[List[Dict[str, Tensor]]], List[Tensor], bool) -> Tuple[Dict[str, Tensor], List[Dict[str, Tensor]], Dict[str, Tensor], bool]
        """
        Arguments:
        ...
        if self.training:
            ...
            for t_data in targets:         # 进一步判断传入的target的boxes参数是否符合规定
                boxes = t_data["boxes"]
        ...
        #tag: yang adds domain results
        if is_target:
            lc_domain_results, gl_domain_results = self.backbone(images.tensors, is_target=is_target)
            return lc_domain_results, gl_domain_results
        else:
            features, lc_domain_results, lc_features, gl_domain_results, gc_features = self.backbone(images.tensors)
        ...
        # if self.backbone.withPA and mask_features is not None: 
        ...
        if self.withRPNMask and masks is not None:
            # tag: yang add features 
            proposals, proposal_losses, features = self.rpn(images, features, targets, masks)
        else:    
        ...
        if len(lc_features) and len(gc_features):
            detections, detector_losses = self.roi_heads(features, proposals, images.image_sizes, targets, lc_features=lc_features, gc_features=gc_features)
        else:
        ...
        if torch.jit.is_scripting():
            ...
            return losses, detections, lc_domain_results, gl_domain_results # detections, 
        else:
            return self.eager_outputs(losses, detections, lc_domain_results, gl_domain_results) # losses, detections, lc_domain_results, gl_domain_results
...
class FasterRCNN(FasterRCNNBase):
    ...
    def __init__(self, backbone, num_classes=None,
                 ...
                 # mask parameters
                 mask_head=None, withRPNMask=False, soft_val=1., lc=False, gl=False, context=False):
        ...
        if rpn_head is None:
            if withRPNMask:
                rpn_head = RPNMaskwithFeaturesHead(
        ...
        if box_roi_pool is None:
            box_roi_pool = MultiScaleRoIAlign(
                ...
                output_size=  [13, 13], # [13, 13], # default  [7, 7] 
                sampling_ratio=2)
        ...
        if box_head is None:
            ...
            # #tag: yang adds lc and gc context
            # if self.lc and self.context:
            #     representation_size += 128
            # if self.gc and self.context:
            #     representation_size += 128
            box_head = TwoMLPHead(
        ...
        if box_predictor is None:
            ...
            #tag: yang adds lc and gc context
            if self.lc and self.context:
                representation_size += 64*2 #out_chanels*layer
            if self.gc and self.context:
                representation_size += 64*2  #out_chanels*layer
