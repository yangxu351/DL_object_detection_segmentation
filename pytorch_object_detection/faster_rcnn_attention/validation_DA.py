...
import argparse
import transforms
from network_files.faster_rcnn_framework_DA import FasterRCNN
...
def main(parser_data, val_all=False):
    ...
    backbone = resnet50_fpn_backbone(norm_layer=torch.nn.BatchNorm2d, lc=parser_data.lc, gl=parser_data.gl, context=parser_data.context)
    model = FasterRCNN(backbone=backbone, num_classes=parser_data.num_classes + 1, lc=parser_data.lc, gl=parser_data.gl, context=parser_data.context)
